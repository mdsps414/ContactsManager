...
public class ContactsListFragment extends BaseRecyclerFragment implements
        LoaderManager.LoaderCallbacks<ArrayList<BaseObject>>{

    final int CNN_CONTACTS_LIST_LOADER = 0;
    ...
    public void onStart() {
        super.onStart();
        getActivity().getSupportLoaderManager().initLoader(CNN_CONTACTS_LIST_LOADER, null, this);
    }
    ...
    public void onLoadFinished(Loader<ArrayList<BaseObject>> loader, ArrayList<BaseObject> data) {
        if(loader.getId() == CNN_CONTACTS_LIST_LOADER){
